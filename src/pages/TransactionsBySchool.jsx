...
const TransactionsBySchool = () => {
  ...
  console.log("the schoolId", schoolId);
  const limit = 3;
  const fetchTransactionsBySchoolId = async (schoolId) => {
    try {
      const response = await getTransactionBySchool(schoolId);
      console.log("the api response", response?.data?.data);
  ...
  useEffect(() => {
    fetchTransactionsBySchoolId(schoolId);
  }, [schoolId]);
